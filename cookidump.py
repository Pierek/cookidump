...
def listToFile(browser, baseDir, category=None):
	"""Gets html from search list and saves in html file"""
	filename = baseDir+ 'index.html' if category is None else baseDir + category + '.html'
...
def run(webdriverfile, outputdir):
	...
	#find all categories and group all recipes by categories in separate folders
	print('[CD] The recipes will be grouped by categories...')
	all_categories_div = brw.find_elements_by_xpath('//div[core-checkbox[input[starts-with(@value, "VrkNavCategory")]]]')

	categories = [
		(
			category_div.find_element_by_tag_name('core-checkbox').find_element_by_tag_name('input').get_attribute('value'),
			category_div.find_element_by_tag_name('label').get_attribute('innerHTML')
		) for category_div in all_categories_div
	]
	...
	#get base url with filter and further add category to the url
	current_base_url = brw.current_url

	for category in categories:

		current_url = current_base_url + '&categories=' + category[0]
		...
		listToFile(brw, outputdir, category[1])
